...
import { useProductIdQuery } from "../../api";

const UserProductPage = () => {
  const { productId } = useParams();
  const location = useLocation();
  const store_discount = location.state;
                ...
                <div className="flex text-center flex-col relative">
                  <span
                    className={`text-lg sm:text-xl lg:text-2xl xl:text-5xl font-semibold ${
                      data.discount || store_discount > 0 ? "line-through" : ""
                    }`}
                  >
                    {data.price}$
                  </span>
                  <span
                    className={`text-green-600 text-xl sm:text-2xl lg:text-3xl xl:text-7xl font-semibold ${
                      data.discount || store_discount > 0 ? "block" : "hidden"
                    }`}
                  >
                    {getDiscountedPrice(data, store_discount)}$
                  </span>
                </div>
              </div>
