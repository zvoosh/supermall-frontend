...
import { useEditStoreMutation, useStoreIdQuery } from "../../api";
...
const EditStorePage = () => {
  ...
  const { mutate: editStoreMutation } = useEditStoreMutation();
  const { data } = useStoreIdQuery(id!);

  useEffect(() => {
    editstore.setFieldsValue(data);
  }, [data]);
  ...
  const onFinish = (values: {
    ...
    let file;
    let payload;
    if (editstore.getFieldsValue().img.file) {
      file = editstore.getFieldsValue().img.file;
              ...
              >
                <Upload
                  ...
                  beforeUpload={(file) => {
                    ...
                    editstore.setFieldsValue({ img: file });
                    return false;
                  }}
                >
